...
class ResidualAttentionBlock_COMMA(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
                 text_layer=False, i=0):
        super().__init__()
        ...
        self.attn_mask = attn_mask
        # This must be consistent with the config file prompt
        self.compound_prompt_nctx = design_details['comma_length']
        if i == 0:
            self.first_layer = True
        else:
            self.first_layer = False
...
class Transformer(nn.Module):
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_MaPLe(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        elif current_trainer == 'COMMA':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_COMMA(width, heads, attn_mask, design_details, text_layer, i)
                  for i in range(layers)])
        else:
    ...
    def forward(self, x: torch.Tensor, last_layer=0):
        if self.current_trainer == 'COMMA' and last_layer!=0:
            x = self.resblocks[:-last_layer](x)
            last_features = []
            for i in range(last_layer, 0, -1):
                x = self.resblocks[-last_layer](x)
                last_features.append(x[0])
            return x, last_features
        elif self.current_trainer == 'CoOp' and last_layer!=0:
            x = self.resblocks[:-last_layer](x)
            last_features = []
            for i in range(last_layer, 0, -1):
                x = self.resblocks[-last_layer](x)
                last_features.append(x)
            return x, last_features
        else:
...
class VisionTransformer_COMMA(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
                 design_details):
        super().__init__()
        self.input_resolution = input_resolution
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "MaPLe":
                ...
                self.visual = VisionTransformer_COMMA(
                    input_resolution=image_resolution,
                    patch_size=vision_patch_size,
                    width=vision_width,
                    layers=vision_layers,
    ...
    def encode_text_last_features(self, text, last_layer):
        x = self.token_embedding(text).type(self.dtype)  # [batch_size, n_ctx, d_model]

        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x, last_features = self.transformer(x, last_layer=last_layer)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)
        ...
        last_features = torch.stack(last_features)
        last_features = last_features.permute(0, 2, 1, 3)  # YLND -> YNLD
        last_features = last_features/last_features.norm(dim=-1, keepdim=True)
